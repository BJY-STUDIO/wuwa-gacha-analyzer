...
项目严格按照鸣潮官方 1.0 唤取规则实现：

| 卡池 | 5 星保底 | 5 星 UP 规则 | 4 星 UP 规则 | 保底继承 |
|------|---------|-------------|-------------|---------|
| 新手唤取 | 50 抽 | 无 | 无 | 独立 |
| 新手自选唤取 | 80 抽 | 必出选定角色 | 无 | 独立 |
| 角色常驻唤取 | 80 抽 | 无 | 无 | 独立 |
| 武器常驻唤取 | 80 抽 | 必出选定武器 | 无 | 独立 |
| 角色活动唤取 | 80 抽 | 50% UP，歪则下次大保底 | 50% UP，歪则下次保底 | 同类池共享 |
| 武器活动唤取 | 80 抽 | 必出 UP 武器 | 50% UP，歪则下次保底 | 同类池共享 |
| 感恩定向唤取 | 80 抽 | 必出选定角色 | 无 | 独立 |
| 角色新旅唤取 | 80 抽 | 50% UP，歪则下次大保底 | 50% UP，歪则下次保底 | 独立（池关清零） |
| 武器新旅唤取 | 80 抽 | 必出 UP 武器 | 50% UP，歪则下次保底 | 独立（池关清零） |
| 角色联动唤取 | 80 抽 | 50% UP，歪则下次大保底 | 50% UP，歪则下次保底 | 同联动主题共享 |
| 武器联动唤取 | 80 抽 | 必出 UP 武器 | 50% UP，歪则下次保底 | 同联动主题共享 |

5 星概率模型（NGA 验证）：基础 0.8%，第 70 抽起软保底（每抽 +9.02%），第 80 抽硬保底。
...
**Q：抓取数据时提示 API 返回错误？**
A：凭证有效期有限（约 24 小时），需要重新抓包获取新的凭证。
